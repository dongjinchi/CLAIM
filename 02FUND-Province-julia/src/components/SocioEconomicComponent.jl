@defcomp socioeconomic begin
	...
	ygrowth           = Parameter(index=[time,regions])
    ...
    function run_timestep(p, v, d, t)
        ...
        if is_first(t)
            ...
            # Calculate income
            for r in d.regions
                oldincome = v.income[t - 1, r] 

                v.income[t, r] = (1 + p.ygrowth[t, r]) * oldincome - p.mitigationcost[t - 1, r]
            end
            ...
            #Calculate income per capita growth rate
            for r in d.regions
                v.ypcgrowth[t, r] = (v.ypc[t,r] - v.ypc[t-1,r]) / v.ypc[t-1,r]
            end
        
        
            for r in d.regions
                v.consumption[t, r] = max(v.income[t, r] * 1000000000.0 * (1.0 - p.savingsrate))
            end
            v.globalconsumption[t] = sum(v.consumption[t,:])
