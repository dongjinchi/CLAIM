...
""" 
function load_default_parameters(nsteps, datadir = joinpath(dirname(@__FILE__), "..", "data"))
...
function prepparameters!(parameters, nsteps)
    for (param_name, p) in parameters
        ...
        elseif column_count == 2
            #lengthp = size(p,1) #<= nsteps+1 ? size(p,1) : nsteps+1
            parameters[param_name] = Float64[getbestguess(convertparametervalue(p[j,2])) for j in 1:size(p,1)]
        elseif column_count == 3
            length_index1 = length(unique(p[:,1])) <= nsteps+1 ? length(unique(p[:,1])) : nsteps+1
