...
function _compute_sc_from_mm(mm::MarginalModel; year::Int, gas::Symbol, ntimesteps::Int, equity_weights::Bool, equity_weights_normalization_region::Int, eta::Float64, prtp::Float64, nregions::Int)
    ...
    syear = getindexfromyear(year)

    # Compute global social cost
    gsc = sum(marginaldamage[syear:ntimesteps, :] .* df[syear:ntimesteps, :])   # need to start from second value because first value is missing
    rsc = sum(marginaldamage[syear:ntimesteps, :] .* df[syear:ntimesteps, :],  dims=1)
    return (gsc=gsc, rsc=rsc,marginaldamage=marginaldamage, df =df)
end
...
function _weight_normalization(gas::Symbol)
    if gas == :CO2 
        return 1e9   # convert from tonnes to Mt since component expects Mt
    elseif gas == :CH4
        return 1e9   # convert from tonnes to Mt since component expects Mt
    elseif gas == :N2O
        return 1e9   # convert from tonnes to Mt since component expects Mt
    elseif gas == :SF6 
        return 1e6   # convert from tonnes to kt since component expects Mt
    else
        error("Unknown gas :$gas.")
    end
end 
...
function add_marginal_emissions!(m, year::Union{Int, Nothing} = nothing; gas::Symbol = :CO2, pulse_size::Float64 = 1e7)
    ...
    elseif gas == :CH4
        connect_param!(m, :emissionspulse, :input, :emissions, :E_ch4)
        connect_param!(m, :climatech4cycle, :E_ch4, :emissionspulse, :output)
    elseif gas == :N2O
        connect_param!(m, :emissionspulse, :input, :emissions, :E_n2o)
        connect_param!(m, :climaten2ocycle, :E_n2o, :emissionspulse, :output)
    elseif gas == :SF6
